...
#define TAG "newEngineTest BuildInfo"
...
BuildInfo::BuildInfo()
    ...
    //ALOGD("BuildInfo j_build_info_ id:%d", j_build_info_);
}

std::string BuildInfo::GetStringFromJava(const char* name) {
  
  AttachCurrentThreadIfNeeded test;
  ALOGD("BuildInfo::GetStringFromJava need attach:%d", test.attached_);
  if(test.attached_) {
    j_environment_ = JVM::GetInstance()->environment();
    j_build_info_ = JVM::GetInstance()->GetClass(
        "org/webrtc/voiceengine/BuildInfo");
  ...
  /*AttachCurrentThreadIfNeeded test;
  ALOGD("BuildInfo::GetStringFromJava need attach:%d", test.attached_);
  rtc::scoped_ptr<JNIEnvironment> _j_environment_ = JVM::GetInstance()->environment();
  JavaClass _j_build_info_ = JVM::GetInstance()->GetClass(
        "org/webrtc/voiceengine/BuildInfo");*/

  jmethodID id = j_build_info_.GetStaticMethodId(name, "()Ljava/lang/String;");
  ALOGD("BuildInfo GetStringFromJava name:%s, id:%d", name, id);
