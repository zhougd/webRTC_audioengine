...
#define TAG "newEngineTest AudioManager"
...
AudioManager::JavaAudioManager::JavaAudioManager(
  ...
  ALOGD("JavaAudioManager init_ id:%d", init_);
  ALOGD("JavaAudioManager dispose_ id:%d", dispose_);
  ALOGD("JavaAudioManager is_communication_mode_enabled_ id:%d", is_communication_mode_enabled_);
  ALOGD("JavaAudioManager is_device_blacklisted_for_open_sles_usage_ id:%d", is_device_blacklisted_for_open_sles_usage_);
}
...
void AudioManager::JavaAudioManager::Close() {
  ALOGD("CallVoidMethod JavaAudioManager::Close id:%d", dispose_);
...
void AudioManager::InitJavaEnvironment() {
  /*
  AttachCurrentThreadIfNeeded test;
  rtc::scoped_ptr<JNIEnvironment> _j_environment_;
  rtc::scoped_ptr<NativeRegistration> _j_native_registration_;
  rtc::scoped_ptr<AudioManager::JavaAudioManager> _j_audio_manager_;
  _j_environment_ = JVM::GetInstance()->environment();
  JNINativeMethod native_methods[] = {
      {"nativeCacheAudioParameters",
       "(IIZZZZIIJ)V",
       reinterpret_cast<void*>(&webrtc::AudioManager::CacheAudioParameters)}};
  _j_native_registration_ = _j_environment_->RegisterNatives(
      "org/webrtc/voiceengine/WebRtcAudioManager",
      native_methods, arraysize(native_methods));
  _j_audio_manager_.reset(new JavaAudioManager(
      _j_native_registration_.get(),
      _j_native_registration_->NewObject(
...
void AudioManager::UnInitJavaEnvironment() {
...
bool AudioManager::Init() {
  ...
  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioManager::Init need attach:%d", test.attached_);
  ...
  if (!_j_audio_manager_->Init()) {
    ALOGE("init failed!");
    //UnInitJavaEnvironment();
    return false;
  }
...
bool AudioManager::Close() {
  ...
  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioManager::Close need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}
...
bool AudioManager::IsCommunicationModeEnabled() {
  ALOGD("IsCommunicationModeEnabled()");
  RTC_DCHECK(thread_checker_.CalledOnValidThread());

  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioManager::IsCommunicationModeEnabled need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}
  ...
  bool resault = _j_audio_manager_->IsCommunicationModeEnabled();
...
bool AudioManager::IsLowLatencyPlayoutSupported() {
  RTC_DCHECK(thread_checker_.CalledOnValidThread());
  ALOGD("IsLowLatencyPlayoutSupported()");

  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioManager::IsLowLatencyPlayoutSupported need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}
  ...
  bool resault = _j_audio_manager_->IsDeviceBlacklistedForOpenSLESUsage() ?
