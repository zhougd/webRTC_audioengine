...
#define TAG "newEngineTest AudioRecordJni"
...
AudioRecordJni::JavaAudioRecord::JavaAudioRecord(
    ...
    : audio_record_(audio_record.Pass()),
      ...
      enable_built_in_ns_(native_reg->GetMethodId(
          "enableBuiltInNS", "(Z)Z")) {
    ALOGD("JavaAudioRecord init_recording_ id:%d", init_recording_);
    ALOGD("JavaAudioRecord start_recording_ id:%d", start_recording_);   
    ALOGD("JavaAudioRecord stop_recording_ id:%d", stop_recording_);       
    ALOGD("JavaAudioRecord enable_built_in_aec_ id:%d", enable_built_in_aec_);   
    ALOGD("JavaAudioRecord enable_built_in_agc_ id:%d", enable_built_in_agc_); 
    ALOGD("JavaAudioRecord enable_built_in_ns_ id:%d", enable_built_in_ns_);   
}
...
bool AudioRecordJniRunFunction(void* obj) {
  //ALOGD("AudioTrackJniRunFunction:%s", GetThreadInfo().c_str());
  AudioRecordJni *record = (AudioRecordJni*)obj;
  record->DoOperacation();
  return true;
}
...
AudioRecordJni::AudioRecordJni(AudioManager* audio_manager)
    : /*j_environment_(JVM::GetInstance()->environment()),*/
      ...
      record_thread_(AudioRecordJniRunFunction, this, "AudioRecordJni thread"),
...
void AudioRecordJni::InitJavaEnvironment() {
  j_environment_ = JVM::GetInstance()->environment();
...
void AudioRecordJni::UnInitJavaEnvironment() {
    //j_native_registration_ = nullptr;
}

static const int kAudioRecordIntervalMs = 10000;
void AudioRecordJni::DoOperacation() {
  AttachCurrentThreadIfNeeded test;
  ALOGD("AudioTrackJni::AudioRecordJniRunFunction need attach:%d", test.attached_);
  if(test.attached_) {
    InitJavaEnvironment();
  }
  while (true) {
    bool shutting_down = operation_event_.Wait(kAudioRecordIntervalMs);
    ...
    if(_currentOperation == 4) {
      EnableBuiltInAECEX(_boolfristarg);
      result_event_.Set();
    }

    if(_currentOperation == 5) {
      EnableBuiltInAGCEX(_boolfristarg);
      result_event_.Set();
    }

    if(_currentOperation == 6) {
      EnableBuiltInNSEX(_boolfristarg);
      result_event_.Set();
    }
...
int32_t AudioRecordJni::InitRecording() {
   ALOGD(" Call InitRecording:%s", GetThreadInfo().c_str());

  _currentOperation = 1;

  operation_event_.Set();
  while(true) {
    bool shutting_down = result_event_.Wait(kAudioRecordIntervalMs);
    if (shutting_down)
        break;
  }

  ALOGD(" Call InitRecording:%s finish", GetThreadInfo().c_str());
  return 0;
}

int32_t AudioRecordJni::InitRecordingEX() {
  ...
  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioRecordJni::InitRecording need attach:%d", test.attached_);
...
int32_t AudioRecordJni::StartRecording() {
   ALOGD(" Call StartRecording:%s", GetThreadInfo().c_str());
  ...
  ALOGD(" Call StartRecording:%s finish", GetThreadInfo().c_str());
  return 0;
}

int32_t AudioRecordJni::StartRecordingEX() {
  ...
  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioRecordJni::StartRecording need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}
...
int32_t AudioRecordJni::StopRecording() {
   ALOGD(" Call StopRecording:%s", GetThreadInfo().c_str());
  ...
  ALOGD(" Call StopRecording:%s finish", GetThreadInfo().c_str());
  return 0;
}

int32_t AudioRecordJni::StopRecordingEX() {
  ...
  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioRecordJni::StopRecording need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}
...
int32_t AudioRecordJni::EnableBuiltInAEC(bool enable) {
   ALOGD(" Call EnableBuiltInAEC:%s", GetThreadInfo().c_str());

  _currentOperation = 4;

  operation_event_.Set();
  _boolfristarg = enable;
  while(true) {
    bool shutting_down = result_event_.Wait(kAudioRecordIntervalMs);
    if (shutting_down)
        break;
  ...
  ALOGD(" Call EnableBuiltInAEC:%s finish", GetThreadInfo().c_str());
  return 0;
}

int32_t AudioRecordJni::EnableBuiltInAECEX(bool enable) {
  ALOGD("EnableBuiltInAEC%s", GetThreadInfo().c_str());
  RTC_DCHECK(thread_checker_.CalledOnValidThread());

  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioRecordJni::EnableBuiltInAEC need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}

  int resault = j_audio_record_->EnableBuiltInAEC(enable) ? 0 : -1;

  return resault;
}

int32_t AudioRecordJni::EnableBuiltInAGC(bool enable) {
   ALOGD(" Call EnableBuiltInAGC:%s", GetThreadInfo().c_str());
  ...
  ALOGD(" Call EnableBuiltInAGC:%s finish", GetThreadInfo().c_str());
  return 0;
}

int32_t AudioRecordJni::EnableBuiltInAGCEX(bool enable) {
  ALOGD("EnableBuiltInAGC%s", GetThreadInfo().c_str());
  RTC_DCHECK(thread_checker_.CalledOnValidThread());

  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioRecordJni::EnableBuiltInAGC need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}

  int resault = j_audio_record_->EnableBuiltInAGC(enable) ? 0 : -1;

  return resault;
}

int32_t AudioRecordJni::EnableBuiltInNS(bool enable) {
   ALOGD(" Call EnableBuiltInNS:%s", GetThreadInfo().c_str());
  ...
  ALOGD(" Call EnableBuiltInNS:%s finish", GetThreadInfo().c_str());
  return 0;
}

int32_t AudioRecordJni::EnableBuiltInNSEX(bool enable) {
  ALOGD("EnableBuiltInNS%s", GetThreadInfo().c_str());
  RTC_DCHECK(thread_checker_.CalledOnValidThread());

  //AttachCurrentThreadIfNeeded test;
  //ALOGD("AudioRecordJni::EnableBuiltInNS need attach:%d", test.attached_);
  //if(test.attached_ || j_native_registration_ == nullptr) {
  //  InitJavaEnvironment();
  //}

  int resault = j_audio_record_->EnableBuiltInNS(enable) ? 0 : -1;
