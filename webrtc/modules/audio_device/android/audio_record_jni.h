...
class AudioRecordJni {
 public:
  ...
  int32_t InitRecordingEX();
  bool RecordingIsInitialized() const { return initialized_; }

  int32_t StartRecording();
  int32_t StartRecordingEX();
  int32_t StopRecording();
  int32_t StopRecordingEX();
  bool Recording() const { return recording_; }

  void AttachAudioBuffer(AudioDeviceBuffer* audioBuffer);

  int32_t EnableBuiltInAEC(bool enable);
  int32_t EnableBuiltInAECEX(bool enable);
  int32_t EnableBuiltInAGC(bool enable);
  int32_t EnableBuiltInAGCEX(bool enable);
  int32_t EnableBuiltInNS(bool enable);
  int32_t EnableBuiltInNSEX(bool enable);

  void DoOperacation();
  void InitJavaEnvironment();
  void UnInitJavaEnvironment();
 ...
 private:
  ...
  rtc::PlatformThread record_thread_;
  rtc::Event operation_event_;
  rtc::Event result_event_;
